...
class MY_Model extends CI_Model
{
	...
	function update_by_slug($slug, $data)
	{
		if (!$slug) {
			return FALSE;
		}
		...
		$this->update_by_slug_rule($where, $data);
	 ...
	 * $data : mang du lieu can cap nhat
	 */
	function update_by_slug_rule($where, $data)
	{
		if (!$where) {
			return FALSE;
		}
	...
	function get_by_slug($slug, $field = '')
	{
		if (!$slug) {
			return FALSE;
		}
		...
		return $this->get_by_slug_rule($where, $field);
	}
	 ...
	 * $field: Cột muốn lấy dữ liệu
	 */
	function get_by_slug_rule($where = array(), $field = '')
	{
		if ($field) {
			$this->db->select($field);
		}
